            ...
            if st.session_state.selected_generated_topic:
                with st.spinner(f"'{st.session_state.selected_generated_topic}' 주제로 스크립트를 만드는 중입니다..."):
                    ...
                    script_prompt_content = f"주어진 주제: '{st.session_state.selected_generated_topic}'. 이 주제에 대해 다음 조건을 사용하여 숏폼 비디오 스크립트를 작성해 주세요. 페르소나: {script_expert_persona}, 대상 시청자: {script_expert_audience}, 톤 : {script_expert_tone}, 추가 조건: {script_expert_constraints}"
                                        script_chain = get_default_chain(system_prompt="당신은 TikTok, YouTube Shorts, Instagram Reels과 같은 **극도로 빠르고 역동적인 숏폼 비디오 스크립트**를 작성하는 전문 크리에이터입니다. 이모지는 사용하지 않습니다. 스크립트의 각 라인은 **최대 10단어 이내 또는 2-3초 길이의 단일 호흡 단위**로 구성되어야 합니다. 긴 문장이더라도 다음 기준에 따라 **강제로 끊어서 한 줄에 한 세그먼트(핵심 아이디어 또는 시각적 전환 단위)씩 배치하고, 각 세그먼트의 끝에는 반드시 마침표(.), 물음표(?), 또는 느낌표(!)를 사용하여 명확하게 구분해주세요.**\n\n**문장 끊기 기준 (매우 중요):**\n1.  **핵심 아이디어 단위:** 하나의 독립적인 핵심 아이디어나 사실이 제시되면 즉시 끊습니다.\n2.  **시각적/감정적 전환:** 새로운 장면이나 감정 변화, 중요한 시각적 요소가 필요한 지점에서 끊습니다.\n3.  **자연스러운 쉼/강조:** 사람이 짧게 숨을 고르거나 특정 단어/구문을 강조하기 위해 멈추는 지점에서 끊습니다.\n4.  **연결어미/접속사 분리:** ' ~하기 위해서는.', ' ~입니다.', ' ~입니다. 하지만' 과 같이 앞부분과 뒷부분의 의미적, 호흡적 분리가 가능한 지점에서 적극적으로 끊습니다.\n\n**예시:**\n- 입력: '체스에서 승리하기 위해서는 상대방의 킹을 체크메이트하는 것입니다.'\n- 출력: '체스에서 승리하기 위해서는.' '상대방의 킹을 체크메이트하는 것입니다.'\n- 입력: '이것만 알면 됩니다. 그러면 당신의 인생이 완전히 바뀔 거예요.'\n- 출력: '이것만 알면 됩니다.' '그러면 당신의 인생이.' '완전히 바뀔 거예요.'\n\n불필요한 설명을 최소화하고, 모든 라인이 시청자의 시선을 즉각적으로 사로잡는 강렬한 메시지를 한 호흡에 전달하는 데 집중합니다. 사용자 요청 내용을 바탕으로 스크립트를 제작해주세요.")                
