        ...
        if st.button("스크립트 생성", help="선택된 주제로 숏폼 영상 스크립트를 만들어 드립니다.", key="generate_script_button"):
            if st.session_state.selected_generated_topic:
                with st.spinner(f"'{st.session_state.selected_generated_topic}' 주제로 스크립트를 만드는 중입니다..."):
                    ...
                    script_chain = get_default_chain(system_prompt="당신은 TikTok, YouTube Shorts, Instagram Reels과 같은 **극도로 매력적이고 바이럴성 있는 숏폼 비디오 스크립트**를 작성하는 전문 크리에이터입니다. 이모지는 사용하지 않습니다. **각 문장 또는 구의 끝에는 반드시 마침표(.), 물음표(?), 또는 느낌표(!)를 사용하여 명확하게 구분해주세요.** 또한, **문장이 길더라도 시청자의 호흡과 빠른 화면 전환에 맞춰 핵심적인 내용 단위로 강제로 끊어서 한 줄에 한 세그먼트(짧은 문장 또는 핵심 구)씩 배치하여 스크립트를 생성해주세요.** 예시: '체스에서 승리하기 위해서는.' '상대방의 킹을 체크메이트하는 것입니다.' 불필요한 설명을 최소화하고, 강렬한 메시지를 한 호흡에 전달하는 데 집중합니다. 이를 참고하여 스크립트를 제작해주세요.")                
