...
또한, 영상 스크립트 생성 및 영상 제작 기능도 제공합니다.
"""
)
...
with st.sidebar:
    ...
    with st.expander("전문가 페르소나 설정", expanded=True):
        st.write("주제 생성을 위한 전문가 AI의 설정을 정의합니다.")
        ...
        expert_tone = st.text_input("톤 (예: 유익한, 재미있는, 심각한)", value=st.session_state.get("expert_tone", "유익한"))
        ...
        if st.button("주제 생성"):
            ...
            with st.spinner("전문가 페르소나가 주제를 생성 중입니다..."):
                st.session_state.messages.append({"role": "user", "content": f"전문가 페르소나({expert_persona})로 '{expert_domain}'에 대한 '{expert_audience}' 대상의 '{expert_tone}' 톤 '{expert_format}' 형식의 주제를 생성해줘. 추가 조건: {expert_constraints}"})
                ...
                if st.session_state.generated_topics:
                    ...
                    st.session_state.messages.append({"role": "assistant", "content": "주제 생성에 실패했습니다. 설정을 확인해주세요."})
            st.rerun()
    ...
    with st.expander("RAG (검색 증강 생성) 설정", expanded=False):
        ...
        url_input = st.text_area(
            "분석할 웹사이트 URL 입력 (쉼표로 구분)",
            placeholder="예: https://www.streamlit.io, https://www.google.com",
        )

        process_button = st.button("문서/URL 처리")
        if process_button:
            if uploaded_files or url_input:
                st.session_state.retriever = get_retriever_from_source(uploaded_files, url_input)
                if st.session_state.retriever:
                    st.success("문서 및 URL 처리가 완료되었습니다!")
                else:
                    st.warning("처리할 유효한 파일이나 URL이 없습니다.")
            else:
                st.warning("파일을 업로드하거나 URL을 입력해주세요.")
    ...
    with st.expander("영상 제작 설정", expanded=True):
        ...
        if st.button("스크립트 생성", help="선택된 주제로 숏폼 영상 스크립트를 생성합니다."):
            if st.session_state.selected_generated_topic:
                with st.spinner(f"'{st.session_state.selected_generated_topic}' 주제로 스크립트를 생성 중입니다..."):
                    ...
                    st.session_state.messages.append({"role": "user", "content": f"선택된 주제 '{st.session_state.selected_generated_topic}'에 대한 스크립트를 생성해줘."})
                ...
                st.rerun() # 스크립트가 업데이트되도록 리런
            else:
                st.warning("먼저 생성된 주제를 선택해주세요.")
        ...
        if uploaded_bgm:
            ...
            st.success(f"배경 음악 '{uploaded_bgm.name}' 업로드 완료!")
        ...
        if st.button("스크립트 -> 오디오 변환"):
            if st.session_state.edited_script_content and st.session_state.include_voice:
                ...
                st.warning("오디오를 생성하려면 스크립트 내용을 입력하고 'AI 목소리 포함'을 체크해야 합니다.")

        if st.button("이미지 생성"):
            if st.session_state.video_topic:
                with st.spinner(f"'{st.session_state.video_topic}' 에 대한 이미지 생성 중..."):
                ...
                st.success("이미지 생성 완료! (assets/image_X.jpg)")
            else:
                st.warning("이미지를 생성하려면 영상 주제를 입력해주세요.")

        if st.button("영상 미리보기"):
            if st.session_state.audio_path and os.path.exists("assets/image_0.jpg"): # 최소 1개 이미지 존재 확인
                with st.spinner("영상 미리보기 생성 중... (오디오와 이미지 동기화)"):
        ...
        if st.button("영상 최종 생성 (자막 포함)"):
            if st.session_state.audio_path and os.path.exists("assets/image_0.jpg"):
                ...
                st.warning("최종 영상을 생성하려면 오디오와 이미지가 먼저 생성되어야 합니다.")
...
# 사용자 입력 처리
if user_input := st.chat_input("메시지를 입력하세요 (예: 최근 AI 기술 트렌드 알려줘, 이 파일 요약해줘, 이 URL 분석해줘)"):
