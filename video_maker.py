...
def create_video_with_segments(
    ...
    ass_path=None,                 # ✅ 추가: 자막 .ass 경로(있으면 자동 번인)
):
    """
    segments: generate_subtitle_from_script가 돌려준 exact_segments(=자막과 완전히 동일한 타이밍)
    ass_path : .ass 파일 경로. 제공되면 렌더 후 ffmpeg로 번인하여 최종 save_path에 저장
    """
    video_width = 720
    video_height = 1080
    clips = []
