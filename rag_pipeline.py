...
- 역할에 맞는 관점과 방식으로 응답하되, **주제를 생성할 때는 반드시 하이픈(-)으로 시작된 리스트 형식으로 출력하라.**
...
def rag_with_sources(inputs: dict):
    ...
    # === 이 프롬프트 부분을 수정합니다 ===
    prompt = ChatPromptTemplate.from_messages([
        ("system", """당신은 질문에 답변하고, 참조 문서에서 관련 정보를 추출해 주는 전문가입니다.

- 만약 주제를 생성하라는 지시를 받으면, 반드시 **하이픈(-)** 으로 시작하는 리스트 형식으로 3개만 출력하세요.
- 각 주제는 최대 10단어 이내의 간결한 한국어 문장 또는 키워드로 구성하세요.
- 다른 설명 없이 오직 리스트만 응답하세요.
- 그 외에는 문맥에 따라 일반적인 요약을 해도 됩니다.
- 답변은 항상 한국어로 하세요.
