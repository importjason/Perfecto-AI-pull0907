...
try:
    ...
    # ssml_converter.py 상단 import 근처
...
def segment_all_in_one(script_text: str, persona_hint: str = "", *, log=False, force_refresh=False):
    """
    대본 전체를 LLM 한 번 호출로:
      - breath_lines: LLM 분절 결과(줄단위 원문 그대로)
      - ssml_by_line: 각 줄에 대응하는 SSML(</speak> 없이 조각만)
      - keywords_by_line: 각 줄의 키워드 리스트(1~2개)
    를 JSON으로 받아온다.
    """
    t = (script_text or "").strip()
    if not t:
        return {"breath_lines": [], "ssml_by_line": [], "keywords_by_line": []}

    # 캐시 키(대본+프롬프트버전+페르소나)
    PROMPT_VERSION = "v3"  # 프롬프트 바꾸면 버전 올리기
    key = _hash_key(PROMPT_VERSION, persona_hint, len(t), t[:280])
    path = os.path.join(CACHE_DIR, f"segpack_{key}.json")

    if not force_refresh and os.path.exists(path):
        with open(path, "r", encoding="utf-8") as f:
            return json.load(f)

    # ── 하나의 프롬프트로 구조화 JSON 요구 ──
    system = (
        "너는 한국어 숏폼 대본 편집기/음성설계자다. "
        "입력 대본을 라인 호흡 단위로 자르고(breath_lines), 각 라인을 Polly 친화 SSML로 만들고(ssml_by_line), "
        "각 라인의 검색용 키워드 1~2개(keywords_by_line)를 뽑아라. "
        "출력은 반드시 JSON만."
    )
    user = f"""
[규칙]
- breath_lines: 원문 보존, 줄바꿈만 추가. 의미 임의 변형 금지.
- ssml_by_line: 해당 라인만을 <prosody>..<break/> 조각으로(</speak> 금지), 숫자/단위/물음표 보존.
- keywords_by_line: 영어 2~5단어 내, 중복 최소화, 라인 의미 핵심.
...
{{
  "breath_lines": ["..."],
  "ssml_by_line": ["<prosody ...>...</prosody>", "..."],
  "keywords_by_line": [["kw1","kw2"], ["kw1"], ...]
}}
"""
    # — LLM 호출(기존 get_default_chain 재사용) —
    chain = get_default_chain(system)
    out = chain.invoke({"question": user})
    if not isinstance(out, str): out = str(out or "")
    out = out.strip()

    # JSON만 남기기(혹시 텍스트 섞이면 중괄호부터 추출)
    m = re.search(r"\{.*\}\s*$", out, flags=re.S)
    payload = out[m.start():] if m else out
    try:
        data = json.loads(payload)
    except Exception:
        # 실패 시 최소 폴백: breath만 LLM으로 따로, SSML/키워드는 휴리스틱
        bl = breath_linebreaks(t, honor_newlines=False, log=log)
        data = {"breath_lines": bl, "ssml_by_line": [convert_line_to_ssml(x) for x in bl], "keywords_by_line": [[x[:40]] for x in bl]}

    # 길이 정합 맞추기
    n = len(data.get("breath_lines", []))
    data["ssml_by_line"] = (data.get("ssml_by_line", []) + [""]*n)[:n]
    data["keywords_by_line"] = (data.get("keywords_by_line", []) + [[]]*n)[:n]

    with open(path, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False)
    if log:
        import streamlit as st
        st.write("🧪 [batch] LLM 1회 호출로 세그먼트/SSML/키워드 수신:", n)
    return data
