...
# 1) 호흡/분절 프롬프트
BREATH_PROMPT = """
역할: 호흡 라인브레이커. 줄바꿈만 조정. 내용/문장부호 불변.

규칙
1) \n만 추가, 빈 줄/양끝 공백 제거. 중복 공백 1칸.
2) 값+단위(6,371킬로미터, -173도, 100배) 분리 금지.
3) 보조용언/문말: “… 수 있/없습니다”, “… 것입니다/이다” 한 줄 유지.
4) 라벨/값: “X는” ⏎ “값…입니다/이다”.
5) 질문 머리말: “왜일까요/무엇일까요/어떻게 될까요” 단독 줄.
6) 전환 표지 단독 허용: {대신, 하지만/그러나, 그리고/또한, 따라서/즉/그러니까, 한쪽으로는/다른 쪽으로는}.
7) 쉼표 뒤 분리 가능(숫자 천단위 쉼표 제외). 명사구/조사 경계 어색 분리 금지.
8) 길이: 8–18자 권장. 1–2단어 라인 남발 금지(강조 단어 예외).

검수: R2–R5 위반 없는지 확인 후 결과만 출력.
